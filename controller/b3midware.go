...
func fillUser(c *gin.Context) {
	...
	if strings.Contains(c.Request.URL.Path, util.PathAPIsSymArticle) || strings.Contains(c.Request.URL.Path, util.PathAPIsSymComment) { // https://github.com/binlihpu/pipe/issues/138
		c.Next()
	...
	case "":
		...
		if "/admin/" == c.Request.URL.Path { // https://github.com/binlihpu/pipe/issues/67
			redirectURL = model.Conf.Server + c.Request.URL.Path
		}
